...
export default function DriversPage() {
  ...
  const loadDrivers = async () => {
    ...
    try {
      ...
      setError(
        getApiErrorMessage(
          loadError,
          "Không thể tải danh sách tài xế. Vui lòng thử lại.",
        ),
      );
    } finally {
      setLoading(false);
  ...
  const handleCreate = async () => {
    if (createStep < 3) {
      if (
        createStep === 1 &&
        (!form.fullName || !form.phone || !form.licenseNumber)
      ) {
        setError("Vui lòng nhập đủ họ tên, số điện thoại và số GPLX.");
        return;
      }
      if (createStep === 2 && !licenseImage) {
        setError("Vui lòng tải lên ảnh GPLX.");
        return;
      }
      setError("");
    ...
    try {
      ...
      if (licenseImage) {
        uploadedUrl = await mediaApi.uploadFile(
          licenseImage,
          "drivers/license",
        );
      }
      ...
      setOpenModal(false);
      setSuccessMsg(
        "Khai báo tài xế mới thành công! Hồ sơ đang chờ Admin duyệt.",
      );
    } catch (saveError) {
      setError(
        getApiErrorMessage(
          saveError,
          "Không thể gửi tài xế để duyệt. Vui lòng kiểm tra hồ sơ rồi thử lại.",
        ),
      );
    } finally {
      setSaving(false);
  ...
  const handleLicenseImageChange = (event) => {
    ...
    if (
      ...
      setError("Ảnh GPLX chỉ nhận JPG, PNG hoặc PDF, dung lượng tối đa 5MB.");
      return;
    }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setError(
        getApiErrorMessage(
          updateError,
          "Không thể cập nhật thông tin tài xế. Vui lòng kiểm tra lại.",
        ),
      );
    } finally {
      setSaving(false);
  ...
  const handleDelete = async (driver) => {
    ...
    try {
      ...
      setError(
        getApiErrorMessage(
          deleteError,
          "Không thể xóa tài xế. Vui lòng thử lại.",
        ),
      );
    }
  };
  ...
  const handleImportCsvFile = async (event) => {
    ...
    try {
      const rows = parseSimpleCsv(await file.text()).map((row) => ({
        ...
        licenseImageFile:
          row.licenseImageFile || row.licenseImage || row.licenseImageUrl || "",
      }));
      if (
        rows.some((row) => !row.fullName || !row.phone || !row.licenseNumber)
      ) {
  ...
  const validateImportDocuments = () => {
    const missing = importRows
      .filter((row) => !findImportDocument(row.licenseImageFile))
      .map((row) => row.fullName);
    if (missing.length) {
      setError(
        `Chưa tìm thấy ảnh hoặc file GPLX của ${missing.slice(0, 2).join(" và ")}${missing.length > 2 ? " và các tài xế liên quan" : ""}. Kiểm tra lại tên file trong CSV.`,
      );
      return false;
    }
    return true;
  ...
  const handleBulkImport = async () => {
    ...
    try {
      ...
      setSuccessMsg(
        `Đã gửi ${importRows.length} hồ sơ tài xế để Admin xét duyệt.`,
      );
      await loadDrivers();
      ...
      setError(
        getApiErrorMessage(
          importError,
          "Không thể gửi danh sách tài xế. Vui lòng kiểm tra lại hồ sơ.",
        ),
      );
    } finally {
      setSaving(false);
    ...
    <Box className="animate-fade-in-up">
          ...
          <Table sx={{ minWidth: 650 }}>
            <TableHead className="bg-slate-50">
              <TableRow>
                ...
                <TableCell className="font-bold text-slate-600" align="right">
                  Thao tác
                </TableCell>
              </TableRow>
            </TableHead>
                ...
                <TableRow key={d.id} hover className="transition-colors">
                  <TableCell>
                    <Box className="flex items-center gap-3">
                      <Avatar
                        sx={{ bgcolor: "#1B4965", width: 32, height: 32 }}
                      >
      ...
      >
        ...
        <Box
          ...
          aria-label="Tiến trình khai báo tài xế"
        >
        ...
        >
          ...
          {createStep === 3 && (
            ...
            >
              ...
              <Typography variant="body2">
                Họ tên: <strong>{form.fullName}</strong>
              </Typography>
              <Typography variant="body2">
                Số GPLX: <strong>{form.licenseNumber}</strong>
              </Typography>
              <Typography variant="body2">
                Ảnh GPLX: <strong>{licenseImage?.name}</strong>
              </Typography>
              <Typography
                variant="caption"
                sx={{ color: "#64748b", display: "block" }}
              >
                Sau khi gửi, tài xế sẽ ở trạng thái Chờ duyệt để Admin kiểm tra
                hồ sơ.
              </Typography>
            </Box>
          )}
          ...
          >
            <strong>Lưu ý:</strong> Tài xế mới thêm sẽ ở trạng thái{" "}
            <strong>Chờ duyệt</strong> trước khi tham gia nhận chuyến.
          </Box>
        </DialogContent>
        <DialogActions
        ...
        >
          ...
          >
            <input
              ...
              type="file"
              accept="image/jpeg,image/png,application/pdf"
              onChange={handleEditLicenseImageChange}
            />
        ...
        <DialogContent className="pt-5">
            ...
            <Box className="space-y-4">
              <Box className="rounded-lg border border-blue-100 bg-blue-50 px-4 py-3">
                ...
                <Typography variant="body2" className="mt-1 text-slate-600">
                  Mỗi dòng CSV là một tài xế. Tên file GPLX ở cột cuối sẽ được
                  đối chiếu ở bước tiếp theo.
                </Typography>
              </Box>
                ...
                <Button
                  ...
                  onClick={() =>
                    downloadCsvTemplate(
                      ...
                      "fullName,phone,licenseNumber,licenseImageFile\nNguyen Van A,0901234567,B2,gplx001.jpg\n",
                    )
                  }
              ...
              >
                ...
                <CardContent className="flex flex-col items-center gap-2 py-8 text-center">
                  ...
                  <Typography variant="caption" className="text-slate-500">
                    Bắt buộc có họ tên, số điện thoại, số GPLX và tên file GPLX.
                  </Typography>
                </CardContent>
              </Card>
              {importRows.length > 0 && (
          ...
          {importStep === 2 && (
            <Box className="space-y-4">
              ...
              <Card
                component="label"
                htmlFor="driver-documents-input-v2"
                variant="outlined"
                className="cursor-pointer border-2 border-dashed bg-slate-50 transition-colors hover:border-[#1B4965]"
              >
                <input
                  id="driver-documents-input-v2"
                  hidden
                  multiple
                  type="file"
                  accept="image/jpeg,image/png,application/pdf"
                ...
                <CardContent className="flex flex-col items-center gap-2 py-8 text-center">
                  ...
                  >
                    {documentFiles.length
                      ...
                      : "Chọn ảnh và file GPLX"}
                  </Typography>
                  <Typography variant="caption" className="text-slate-500">
                    Mỗi file tối đa 5MB. Hệ thống sẽ đối chiếu đủ file cho từng
                    tài xế.
                  </Typography>
                </CardContent>
              </Card>
          ...
          {importStep === 3 && (
            <Box className="space-y-4">
              <Box className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
                ...
                <Typography variant="body2" className="mt-1 text-slate-600">
                  {importRows.length} tài xế sẽ được tạo ở trạng thái Chờ duyệt.
                  Admin sẽ kiểm tra từng bộ hồ sơ.
                </Typography>
              </Box>
              <Box className="max-h-60 space-y-2 overflow-y-auto">
                {importRows.map((row) => (
                  <Box
                    key={row.fullName}
                    className="grid grid-cols-1 gap-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm sm:grid-cols-2"
                  >
                    <Typography className="font-semibold text-[#1B4965]">
                      {row.fullName}
                    </Typography>
                    <Typography className="truncate text-slate-600">
                      GPLX: {row.licenseImageFile}
                    </Typography>
                  </Box>
                ))}
              </Box>
        ...
        {selectedDriver && (
          <Box className="space-y-6">
            <Box className="flex items-center gap-4">
              <Avatar
                sx={{
                  bgcolor: selectedDriver.active ? "#1B4965" : "#94A3B8",
                  width: 64,
                  height: 64,
                }}
              >
              ...
              <Box>
                <Typography variant="h5" className="font-bold text-slate-800">
                  {selectedDriver.name || selectedDriver.fullName}
                </Typography>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12}>
                ...
                <Typography
                  variant="body1"
                  className="font-medium text-[#1B4965] font-mono font-bold"
                >
