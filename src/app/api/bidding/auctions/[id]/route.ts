...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const res = await fetch(`${gatewayUrl}/api/v1/bidding/auctions/${id}`, {
      method: 'GET',
      headers: { 
    ...
    if (!res.ok) {
        return NextResponse.json({ success: false, message: data?.message || 'Lỗi khi tải chi tiết đấu giá' }, { status: res.status });
    }
