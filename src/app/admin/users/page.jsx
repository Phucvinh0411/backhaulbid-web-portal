...
export default function AdminUsersPage() {
  ...
  const handleStatusChange = async (status) => {
    ...
    try {
      const updated = await identityApi.updateAdminAccountStatus(
        selectedUser.id,
        status,
      );
      setUsers((current) =>
    ...
    <AdminPageShell>
      ...
      <AdminSectionCard sx={{ flex: 1 }}>
        <AdminToolbar
          ...
          subtitle={`${filteredUsers.length} hồ sơ phù hợp`}
        >
          <AdminSearchField
            value={searchQuery}
            onChange={(event) => {
        ...
        <AdminTableContainer minWidth={960}>
          <Table aria-label="Danh sách người dùng">
            <TableHead sx={{ bgcolor: "rgba(27, 73, 101, 0.04)" }}>
              <TableRow>
                {[
                  { id: "displayName", label: "Người dùng" },
                  { id: "email", label: "Liên hệ" },
                  { id: "role", label: "Vai trò" },
                  { id: "registeredAt", label: "Ngày đăng ký" },
            ...
            <TableBody>
              {paginatedUsers.map((row) => (
                <TableRow key={row.id} hover>
                  ...
                  <TableCell>
                    <AdminStatusChip
                      label={typeLabels[row.role] || row.role}
                      tone="neutral"
                    />
                  </TableCell>
                  ...
                  <TableCell align="right">
                    <IconButton
                      ...
                      aria-label={`Xem ${row.displayName}`}
                    >
      ...
      >
          ...
          {selectedUser && (
            <Grid container spacing={3}>
              <Grid item xs={12} md={6}>
                ...
                >
                  {[
                    ["Tên hiển thị", selectedUser.displayName],
                    ["Email", selectedUser.email || "Chưa có"],
                    ["Số điện thoại", selectedUser.phone],
                    [
                      "Vai trò",
                      typeLabels[selectedUser.role] || selectedUser.role,
                    ],
                    ["Ngày đăng ký", formatDate(selectedUser.registeredAt)],
                  ].map(([label, value]) => (
                    <Box
                      key={label}
                      sx={{
              ...
              <Grid item xs={12} md={6}>
                ...
                >
                  <Typography variant="body2">
                    Trạng thái eKYC/doanh nghiệp:{" "}
                    <strong>
                      {selectedUser.verificationStatus || "Chưa có dữ liệu"}
                    </strong>
                  </Typography>
                  <Typography variant="body2" sx={{ mt: 1 }}>
                    Doanh nghiệp:{" "}
                    <strong>
                      {selectedUser.companyName || "Cá nhân hoặc chưa khai báo"}
                    </strong>
                  </Typography>
                </Box>
              </Grid>
        ...
        >
          <AdminSecondaryButton onClick={() => setOpenDialog(false)}>
            Đóng
          </AdminSecondaryButton>
          {selectedUser && selectedUser.status !== "ACTIVE" && (
            ...
            >
              {actionLoading ? "Đang cập nhật..." : "Kích hoạt tài khoản"}
            </AdminPrimaryButton>
          )}
          {selectedUser && selectedUser.status === "ACTIVE" && (
            <AdminPrimaryButton
            ...
            >
              {actionLoading ? "Đang cập nhật..." : "Khóa tài khoản"}
            </AdminPrimaryButton>
          )}
        </DialogActions>
      </AdminDialog>
